...
You're calling {{customer_name}} because you came across their company and saw they're doing great work and could benefit from conversational AI automation.`;
  ...
  const loadVoices = async () => {
    try {
      ...
      // Import voiceService to get real API voices
      const { voiceService } = await import('../lib/voiceService');
      
      // Get real voices from API
      const [regularVoices, premiumVoices] = await Promise.all([
        voiceService.getRegularVoices(),
        voiceService.getPremiumVoices()
      ]);
      
      // Combine and format voices for FlowDesigner compatibility
      const allVoices = [
      ...
      console.log(`✅ ${allVoices.length} real API voices loaded`);
      
    } catch (error) {
      console.error('❌ Failed to load API voices:', error);
      showNotification('Failed to load voices from API', 'error');
  ...
  const testPathway = useCallback(() => {
    showNotification('Web client started!', 'success');
    closeModal();
  }, [showNotification]);
...
Your goal is to establish credibility and guide interactions with confident expertise.`,
  ...
  const sidebarItems = [
    ...
    { icon: '🧪', label: 'Web Client', action: () => showModal('testPathway') },
